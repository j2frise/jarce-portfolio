
{
    ...
    class GridItem {
        constructor(el) {
			...
			this.DOM.bg = this.DOM.el.querySelector('.grid-item-bg');
			// The following DOM elements are elements that will move/tilt when hovering the item.
			this.DOM.tilt = {};
			// The image.
			this.DOM.imgWrap = this.DOM.el.querySelector('.grid-item-wrap');
			this.DOM.tilt.img = this.DOM.imgWrap.querySelector('img');
			// The title (vertical text).
			//this.DOM.tilt.title = this.DOM.el.querySelector('.grid-item-title');
			// The number (horizontal letter/number code).
			this.DOM.tilt.number = this.DOM.el.querySelector('.grid-item-number');
		...
		toggleTexts(delay, withAnimation, show = true) {
			TweenMax.to([this.DOM.tilt.number], !withAnimation ? 0 : show ? 1 : 0.5, {
    ...
    class Content {
        constructor(el) {
            ...
            this.DOM.img = this.DOM.el.querySelector('.content-item-img');
            this.DOM.title = this.DOM.el.querySelector('.content-item-title');
            this.DOM.subtitle = this.DOM.el.querySelector('.content-item-subtitle');
    ...
    class Grid {
        constructor(el) {
			...
			// The grid items.
            this.items = [];
            Array.from(this.DOM.el.querySelectorAll('.grid-item')).forEach(itemEl => this.items.push(new GridItem(itemEl)));
            // The total number of items.
			this.itemsTotal = this.items.length;
			// The content items.
			this.contents = [];
			Array.from(document.querySelectorAll('.content > .content-item')).forEach(contentEl => this.contents.push(new Content(contentEl)));
			// Back control and scroll indicator (elements shown when the item´s content is open).
			this.DOM.closeCtrl = document.querySelector('.content-close');
			this.DOM.scrollIndicator = document.querySelector('.content-indicator');
			// The open grid item.
			this.current = -1;
            // Init/Bind events.
            this.initEvents();
		...
		openItem(item) {
			...
			// Get the "grid-item-bg" width and height and set it explicitly, 
			...
			// Set it to current.
			contentEl.DOM.el.classList.add('content-item-current');
		...
		closeItem(withAnimation = true) {
			...
			TweenMax.to(item.DOM.bg, withAnimation ? 1.2 : 0, {
				...
				rotation: 0,
				onComplete: () => {
					contentEl.DOM.el.classList.remove('content-item-current');
